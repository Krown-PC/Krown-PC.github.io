...
const SERVICES = [
  ...
  { name:"Windows", description:"Instalación y optimización de Windows cuando el proyecto lo requiere." },
  ...
  { name:"Instalación", description:"Instalación de componentes, RGB y accesorios para completar o actualizar el equipo." },
...
function initWhatsApp() {
  document.querySelectorAll(".js-whatsapp").forEach(link => {
    ...
    if (key !== "general") message = `Hola Krown PC's, estoy interesado en el servicio de ${key}. Me gustaría obtener más información y un presupuesto.`;
  ...
  root.innerHTML = EQUIPMENT.map(item => `
    <article class="equipment-card">
      ...
      <div class="equipment-info">
        ...
        ${item.status === "available" && item.price ? `<p><strong>${item.price}</strong></p><a class="button button-primary js-equipment-wa" href="#">Consultar disponibilidad <span>↗</span></a>` : `<p class="section-intro">Proyecto anterior. No disponible.</p>`}
      </div>
    </article>
  `).join("");
  root.querySelectorAll(".js-equipment-wa").forEach((link, i) => {
    const item = EQUIPMENT[i];
    link.href = whatsappUrl(`Hola Krown PC's, estoy interesado en el equipo ${item.name} y quisiera consultar su disponibilidad.`);
    link.target = "_blank";
    link.rel = "noopener noreferrer";
  });
}
